<template>
  <div>
          ...
          <div class="addr-list-wrap">
            <div class="addr-list">
              <ul>
                ...
                <li class="addr-new" @click="addNewFlag=true">
    ...
    <div class="md-modal modal-msg md-modal-transition" v-bind:class="{'md-show':addNewFlag}">
      <div class="md-modal-inner">
        <div class="md-top">
          <div class="md-title" @click="addNewFlag=true">New address</div>
          <button class="md-close" @click="addNewFlag=false">Close</button>
        </div>
        <div class="md-content">
          <div class="confirm-tips">
            <div class="error-wrap">
              <span class="error error-show" v-show="errorTip">Something missing</span>
            </div>
            <ul>
              <li class="regi_form_input">
                <input type="text" tabindex="1" name="userName" v-model="userName" class="regi_login_input regi_login_input_left" placeholder="User Name" data-type="string">
              </li>
              <li class="regi_form_input noMargin">
                <input type="text" tabindex="2" name="streetName" v-model="streetName" class="regi_login_input regi_login_input_left" placeholder="Street Name">
              </li>
              <li class="regi_form_input noMargin">
                <input type="text" tabindex="3" name="postCode" v-model="postCode" class="regi_login_input regi_login_input_left" placeholder="Post Code">
              </li>
              <li class="regi_form_input noMargin">
                <input type="number" tabindex="4" name="tel" v-model="tel" class="regi_login_input  regi_login_input_left" placeholder="Tel Num">
              </li>
            </ul>
          </div>
          <div class="login-wrap">
            <a href="javascript:;" class="btn-login" @click="addAddress">ADD</a>
          </div>
        </div>
      </div>
    </div>
    <modal :mdShow="isMdShow" @close="closeModal">
      ...
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="deleteConfirm">YES</a>
        <a class="btn btn--m btn--red" href="javascript:;" @click="isMdShow=false">NO</a>
      </div>
    </modal>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      addAddress(){
        let params = {
          addressId : this.addressList.length+10001,
        ...
        if (!this.userName || !this.streetName) {
          this.errorTip = true;
          return;
        }
        axios.post("users/addAddress",params).then((response)=>{
          let res = response.data;
          console.dir(res);
          if (res.status === "0") {
            this.errorTip = false;
